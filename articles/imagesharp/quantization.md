...
The default [`Quantize()`](xref:SixLabors.ImageSharp.Processing.QuantizeExtensions.Quantize*) overload uses [`KnownQuantizers.Octree`](xref:SixLabors.ImageSharp.Processing.KnownQuantizers.Octree), which is a fast, good general-purpose adaptive quantizer.
...
- `KnownQuantizers.Octree` for a fast adaptive quantizer with solid general results.
...
- [`OctreeQuantizer`](xref:SixLabors.ImageSharp.Processing.Processors.Quantization.OctreeQuantizer) for fast adaptive quantization.
